...
describe('useFilterRadio', () => {
  test('Should return type "checkbox"', () => {
    const { result } = renderHook(() => useFilterCheckbox(
      {
        source: 'categories',
        name: 'smartphones',
      }
  ...
  test('The checked value should update with the context', () => {
    ...
    act(() => {
      result.current.onChange({ target: { checked: true, name: 'smartphones' }} as any);
    })
  ...
  test('The onChange should call the callback onQueryFilterChange by modifying only the filter concerned', () => {
    ...
    expect(result.current.checked).toBe(true);
    expect(mockCallback.mock.calls[0][0]).toEqual({ brand: 'apple', categories: ['laptops', 'smartphones'] });

    act(() => {
      result.current.onChange({ target: { checked: false, name: 'smartphones' }} as any);
    })

    expect(result.current.checked).toBe(false);
    expect(mockCallback.mock.calls[1][0]).toEqual({ brand: 'apple', categories: ['laptops'] });
  });
});
