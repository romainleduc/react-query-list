    ...
    const { getByText } = render(
      ...
      >
        {({ setFilterValues }: QueryListContextType) => (
          <button onClick={() => setFilterValues?.({ brand: 'apple' })}>
            Filter
          </button>
        )}
      </QueryListProvider>
  ...
  test('The setFilterValues method should remove filters that do not have a truthy value', () => {
    ...
    const { getByText } = render(
      ...
      >
        {({ setFilterValues }: QueryListContextType) => (
          <>
            <button onClick={() => setFilterValues?.({ color: null })}>
              Filter 1
            </button>
            <button onClick={() => setFilterValues?.({ color: undefined })}>
              Filter 2
            </button>
            <button onClick={() => setFilterValues?.({ color: '' })}>
              Filter 3
            </button>
          </>
        )}
